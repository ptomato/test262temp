  ...
  for (const file of files) {
    ...
    for (const test of data) {
      ...
      if (test.result) {
        ...
        if (!testResults.get(normalizedPath)[engineKey]) {
          testResults.get(normalizedPath)[engineKey] = {};
        }
        testResults.get(normalizedPath)[engineKey][scenario] = {
...
function formatCell(result) {
  ...
  if (sloppy && strict) {
    ...
    if (sloppy.pass === true && strict.pass === false) {
      return formatMixed('Passed in sloppy mode but failed in strict mode', strict.message);
    }
    return formatMixed('Failed in sloppy mode but passed in strict mode', sloppy.message);
  }
...
function formatMixed(prefix, message) {
  if (!message) return '⚠️';
  return `[⚠️](## "${sanitizeTooltip(`${prefix}: ${message}`)}")`;
}
